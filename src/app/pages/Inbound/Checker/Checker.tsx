...
const Check = ({
    ...
    return(
        ...
        >
            <span className="text-sm  text-neutral-100"> Item ID: {check_id} | Status:</span><span className={`${column == 'Reject'? 'text-red-600':column == "Checked"? 'text-green-600':'text-gray-500'} ml-3 text-md text-bold  `}>{column}</span>
        </motion.div>
        </>
    )
}
